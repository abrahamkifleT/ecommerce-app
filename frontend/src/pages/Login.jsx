    ...
    <form onSubmit={onSubmitHandler} className='flex flex-col w-[90%] sm:max-w-96 m-auto mt-14 gap-4 text-gray-800'>
      ...
      </div>
      {
        currentState === 'Sign Up' ? (
          <input type="text" className='w-full px-3 py-2 border border-gray-300' placeholder='Name' required />
        ) : ''
      }
      ...
      <div className='w-full flex justify-between text-sm mt-[-8px]'>
        <p className='cursor-pointer underline text-blue-300'>Forgot your Password?</p>
        {
          currentState === 'Login' ?
            <p onClick={() => setCurrentState('Sign Up')} className='cursor-pointer underline text-blue-300'>Create account</p> :
